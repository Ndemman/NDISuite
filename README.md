...
2. Set up environment variables:
   ...
   cp .env.example .env
   ```
   Edit the `.env` file to configure your environment variables, including your OpenAI (`OPENAI_API_KEY`) or Gemma (`GEMMA_API_KEY`) API keys.

3. **AI API Requirements**:
   - **OpenAI**
     - API Key: `OPENAI_API_KEY`
     - Library: `openai>=1.66.0`
   - **Gemma / Google Generative AI**
     - API Key: `GEMMA_API_KEY`
     - Library: `google-generativeai==0.4.*`
     - Optional model override: `GENERATION_MODEL_GEMMA` (defaults to `gemma-27b`)
