"""
Django management command to migrate users from custom model to Django's built-in User model.
"""
...
class Command(BaseCommand):
    help = 'Migrates users from custom users_user model to Django auth_user model'
        ...
        with connection.cursor() as cursor:
            ...
            if not cursor.fetchone():
                ...
                if cursor.fetchone():
                    ...
                    ''')
                    self.stdout.write(self.style.SUCCESS("Created default admin user: admin@example.com with password 'adminpassword'"))
                ...
                self.stdout.write(self.style.SUCCESS('Added auth migrations record'))
            else:
