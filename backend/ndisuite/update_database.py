"""
Direct script to update the database schema with auth_user table.
This script can be run directly using:
python manage.py shell < ndisuite/update_database.py

This bypasses the management command system in case of issues.
"""
import os
import django
import logging
    ...
    with transaction.atomic():
        ...
        with connection.cursor() as cursor:
            ...
            if not cursor.fetchone():
                ...
                if cursor.fetchone():
                    print('Migrating users from users_user to auth_user')
                    ...
                    print(f'Migrated {len(users)} users to auth_user table')
                else:
                    print('No users_user table found. Creating a default admin user.')
                    ...
                    ''')
                    print("Created default admin user: admin@example.com with password 'adminpassword'")
                ...
                print('Added auth migrations record')
            else:
                print('auth_user table already exists. No migration needed.')
