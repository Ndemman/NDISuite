...
const SIDEBAR_WIDTHS = {
  collapsed: 'w-16',
  expanded: 'w-56', // Reduced from w-64 to make it more compact
  mobileCollapsed: 'w-0',
  mobileExpanded: 'w-56',
};
...
export const AppLayout: React.FC<AppLayoutProps> = ({ children }) => {
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden">
      ...
      <aside className={`h-screen fixed left-0 top-0 bottom-0 z-50 bg-card border-r border-border shadow-sm transition-all duration-300 ${isSidebarExpanded ? SIDEBAR_WIDTHS.expanded : SIDEBAR_WIDTHS.collapsed}`}>
        {/* Sidebar Component */}
        <Sidebar showHeader={true} />
      </aside>
      
      {/* Main Content */}
      <div className={`flex-1 flex flex-col overflow-hidden transition-all duration-300 ${isSidebarExpanded ? 'ml-56' : 'ml-16'}`}>
